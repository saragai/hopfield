...
if __name__ == "__main__":
    ...
    for q in range(0, num_pattern):
        ...
        for qi in range(q+1):
            ...
            if qi == q:
                axs[0, q].plot(noise_rates, accs, color=color_array[qi], label=f"image {qi}")
            else:
                axs[0, q].plot(noise_rates, accs, color=color_array[qi])
            axs[1, q].plot(noise_rates, sims, color=color_array[qi])

        axs[0, q].set_title(f"{q+1} Images Accuracy rate")
        axs[1, q].set_title(f"{q+1} Images Similarity")
        # axs[0, q].set_xlabel("noise rate")
        axs[1, q].set_xlabel("noise rate")
        axs[0, q].set_ylim(0., 1.)
        axs[1, q].set_ylim(0., 5)
